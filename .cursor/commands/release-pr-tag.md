...
- 推送 `v*.*.*` 会触发 `.github/workflows/release.yml`：**多平台 `tauri build`**、上传安装包与 `latest.json`（应用内更新），并 **生成 Release 说明**（`generateReleaseNotes`）。需先在仓库 Actions Secrets 配置 `TAURI_SIGNING_PRIVATE_KEY`（详见 [docs/桌面应用-Release与自动更新.md](../../docs/桌面应用-Release与自动更新.md)）。
