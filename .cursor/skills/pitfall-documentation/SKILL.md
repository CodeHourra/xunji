...
- **目录**：优先 `docs/踩坑/`（与仓库现有文档一致）。索引与命名约定见同目录 [README.md](../../../docs/踩坑/README.md)。
- **新文件 vs 合并**：
  - 与已有某篇 **同一主题线**（如同一模块、同一子系统）→ **打开该文件**，在文末或合适位置 **新增小节**，并必要时更新文件顶部摘要句。
  - 新领域或主题正交 → **新建** Markdown 文件。
- **文件名**：**以中文为主**概括主题，用连字符 `-` 分段（例：`桌面应用-脚手架-UnoCSS-Tauri与存储层常见问题.md`、`工具结果-正则匹配失败.md`）。技术专名可保留英文：`UnoCSS`、`Tauri`、`CLI`、`JSON`、`WebView`、`Tailwind`、`FTS5`、`MATCH` 等。
